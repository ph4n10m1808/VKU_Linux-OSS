...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "private_network", ip: "192.168.56.150", name: "vboxnet0"
  ...
  # Config disk size in VirtualBox
  ...
  config.vm.provision "shell", inline: <<-SHELL
     ...
     apt update 
     apt full-upgrade -y
     apt install -y elasticsearch kibana logstash default-jre default-jdk
